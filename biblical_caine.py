#!/usr/bin/env python3
"""
Biblical Man Content in Michael Caine's Voice
"""

biblical_quotes = [
    "You were only supposed to blow the bloody doors off... of your marriage assumptions.",
...
# Format the quotes for both Python and Markdown
quotes_python = repr(biblical_quotes)
quotes_md = "\n\n".join([f"{i+1}. \"{q}\"" for i, q in enumerate(biblical_quotes)])

# Replace placeholders
final_commands = setup_commands.replace("{biblical_quotes}", quotes_python)
final_commands = final_commands.replace("{quotes_md}", quotes_md)
